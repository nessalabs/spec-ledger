...
test('the in-page workflow retains its picker and process state when live data changes workstream',()=>{
 ...
 for(const text of ['Picker W-12','Chosen workflow','Disconnected','current process requirements','No agent session'])assert.ok(html.includes(text),text)
})
test('a deleted library name stays visible with its preserved version while switching is pending',()=>{
 const workflow={profile:{id:'gone',title:'Copied workflow',source:'library',profileDigest:'12345678abcdef',snapshotDigest:'snapshot'}}
 ...
 const tree=Picker({workstreamId:'W-copy',workflow})
 const html=require('react-dom/server').renderToStaticMarkup(tree)
