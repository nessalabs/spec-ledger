...
test('identity presentation breaker: duplicate titled specs keep separate targets without UUID prose or tooltips',()=>{
 ...
 const html=h.render(WorkstreamsList,{workstreams},labels)
 assertHuman(html);assert.match(displayed(html),/Inspect related work Keep the same title/);assert.equal((html.match(/<a /g)??[]).length,2)
