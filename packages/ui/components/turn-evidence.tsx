...
export function TurnEvidence({ initial, turn }: { initial: SessionProjection; turn: Turn }) {
  ...
  return <section className="space-y-4" aria-label="Evidence for this change">
    <p className="text-sm text-muted-foreground">Current checks for this change are shown below. Artifacts record what was observed during <ReadableText>{turn.id}</ReadableText>; they are not proof that later code still works.</p>
