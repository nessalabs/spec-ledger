...
export function SpecSections({ evidence, changes, process, title, workflowHref }: { evidence: ReactNode; changes: ReactNode; process: ReactNode; title?: string; workflowHref?: string }) {
  ...
  useEffect(() => {
    ...
    if (["#evidence", "#changes", "#process"].includes(window.location.hash)) {
  ...
  return <div className="min-w-0 space-y-5">
    <div className="sticky -top-6 z-10 space-y-2 border-b border-border bg-background py-3">
      ...
      {([['evidence', 'Evidence'], ['changes', 'Changes'], ['process', 'Process details']] as const).map(([id, label]) => <a key={id} href={id === "process" && workflowHref ? workflowHref : `#${id}`} onClick={event => { if (id === "process" && workflowHref) return; if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.preventDefault(); window.history.pushState(null, "", `#${id}`); setSelected(id) }} aria-current={selected === id ? "location" : undefined} className={`rounded-lg px-2 py-2 text-center text-xs sm:px-4 sm:text-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring ${selected === id ? 'bg-secondary text-secondary-foreground' : 'text-muted-foreground hover:bg-muted'}`}>{id === "process" && workflowHref ? "Workflow ↗" : label}</a>)}
      </nav>
    </div>
