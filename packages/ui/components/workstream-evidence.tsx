...
export function WorkstreamEvidence({ session, observedAt, expandChecks = false }: { session: Session; observedAt: string; expandChecks?: boolean }) {
  return <section className="space-y-5" aria-label="Verification and evidence">
    ...
    {session.visualEvidence && !session.visualEvidence.ok && <section role="status" className="space-y-2 rounded-xl border border-amber-500/40 bg-amber-500/5 p-4"><h3 className="text-sm font-medium">Screenshots required</h3><p className="text-sm">Attach current screenshots of all relevant UI before finishing this work.</p>{session.visualEvidence.surfaces.filter(s => !s.satisfied).map(s => <p key={`${s.sliceId}/${s.surface}`} className="text-sm text-muted-foreground">Missing: {s.surface}</p>)}{!session.visualEvidence.surfaces.length && <p className="text-sm">The plan needs valid screenshot requirements.</p>}</section>}
