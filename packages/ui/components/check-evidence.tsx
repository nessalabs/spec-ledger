  ...
  return <details open={open} onToggle={event => setOpen(event.currentTarget.open)} className="rounded-xl border border-border p-4">
    ...
    <div className="mt-4 space-y-5">
      {!evidence ? <p role="status">{message || 'Loading saved evidence…'}</p> : <>
        {/* Lead with what actually happened; the definition is one disclosure away. */}
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="outline">{evidence.test?.level ?? 'Test level not recorded'}</Badge>
          <span className="text-sm">Current evidence: {evidence.currentOutcome}</span>
          <span className="ms-auto flex flex-wrap gap-2">
            <Button disabled={starting || active(run) || !evidence.command || !evidence.sourceDigest} onClick={() => void start()}>{starting ? 'Starting…' : active(run) ? 'Running…' : pending.current ? 'Reconnect to request' : 'Run again'}</Button>
            <Button variant="outline" onClick={() => void refresh()}>Refresh evidence</Button>
          </span>
        </div>
        {message && <p role="status" className="text-sm">{message}</p>}
        {run ? <><p className="text-sm text-muted-foreground">{run.sourceDigest !== evidence.sourceDigest || run.checkDigest !== evidence.checkDigest ? "Historical run: its source or check differs from the current version. This result does not verify the current version." : "This run used the source and check identities recorded for this check."}</p><RunOutput run={run}/></> : <p className="text-sm">No detailed run output was captured. Older exit-status records do not contain logs; a new run can capture them.</p>}

        <details className="rounded-lg border border-border/60 p-3">
          <summary className="cursor-pointer text-sm text-muted-foreground">What this check is and how it runs</summary>
          <div className="mt-4 space-y-5">
            ...
            <section className="space-y-2"><h4 className="font-medium">Saved command</h4>{evidence.command ? <CodeBlock code={evidence.command} language="shell" mode="dark" wrap /> : <p className="text-sm">This check is supplied by an external reporter and cannot be run here.</p>}<p className="break-all text-xs text-muted-foreground">Directory: {evidence.cwd}</p><p className="text-xs text-muted-foreground">Run again executes this saved repository command. Opening or refreshing evidence does not execute it.</p></section>
            ...
            <p className="break-all text-xs text-muted-foreground">Current source: {evidence.sourceDigest ?? 'Unavailable'}<br/>Check: {evidence.checkDigest}<br/>Test source: {evidence.source.sha256 ?? 'Not recorded'}</p>
          </div>
        </details>
      </>}
    </div>
...
function RunOutput({run}: {run: CheckRun}) {
  const streams = (['stdout', 'stderr'] as const).filter(key =>
    key === 'stdout' || active(run) || (run[key] && (run[key]!.status !== 'intact' || (run[key]!.text ?? '') !== '')),
  )
  return <section className="space-y-3" aria-label="Actual test output"><h4 className="font-medium">Actual run result · {run.state}</h4><p className="text-sm">{run.outcome ?? 'No outcome yet'}{run.exitCode !== undefined ? ` · exit ${run.exitCode}` : ''}{run.durationMs !== undefined ? ` · ${run.durationMs} ms` : ''}</p><p className="text-xs text-muted-foreground">Started {run.startedAt}{run.finishedAt ? ` · Finished ${run.finishedAt}` : ''}</p>{run.reason && <p className="text-sm">{run.reason}</p>}{streams.map(key => <section key={key}><h5 className="text-sm font-medium">{key === 'stdout' ? 'Standard output' : 'Error output'}{run[key]?.truncated ? ' · truncated' : ''}</h5><pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-muted/30 p-3 text-xs">{run[key]?.status === 'intact' ? run[key]?.text || '(empty)' : active(run) ? 'Output will be available when this run finishes.' : 'Output not captured or its integrity could not be verified.'}</pre></section>)}<details><summary className="cursor-pointer text-xs text-muted-foreground">Run provenance</summary><p className="mt-2 break-all text-xs">Run: {run.runId}<br/>Source: {run.sourceDigest}<br/>Check: {run.checkDigest}<br/>Directory: {run.cwd}</p><pre className="whitespace-pre-wrap break-words text-xs">{run.command}</pre></details></section>
}
