...
export function CheckEvidencePanel({ bindingId, initial, defaultOpen = false, label = "Inspect test, output and source", embedded = false }: { bindingId: string; initial?: CheckEvidence; defaultOpen?: boolean; label?: string; embedded?: boolean }) {
  ...
  return <Container open={embedded ? undefined : open} onToggle={event => { if ("open" in event.currentTarget) setOpen(event.currentTarget.open) }} className={embedded ? "space-y-4" : "rounded-xl border border-border p-4"}>
    {!embedded && <summary className="cursor-pointer font-medium"><ReadableText>{label}</ReadableText></summary>}
    <div className="mt-4 space-y-5">
      {!evidence ? <p role="status">{message || 'Loading saved evidence…'}</p> : <>
        <div className="flex flex-wrap items-center gap-2"><Badge variant="outline">{evidence.test?.level ?? 'Test level not recorded'}</Badge>{evidence.currentOutcome !== "pass" && <span className="text-sm">Current evidence: {evidence.currentOutcome}</span>}</div>
<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0 flex-1"><h4 className="font-medium">{evidence.test?.description ?? "Recorded check"}</h4></div>          <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={starting || active(run) || !evidence.command || !evidence.sourceDigest} onClick={() => void start()}>{starting ? 'Starting…' : active(run) ? 'Running…' : pending.current ? 'Reconnect to request' : 'Run again'}</Button><Button size="sm" variant="ghost" onClick={() => void refresh()}>Refresh evidence</Button></div></div>

        {message && <p role="status" className="text-sm"><ReadableText>{message}</ReadableText></p>}
...
function TextBlock({title, text}: {title: string; text: string}) {
  return <section><h4 className="text-sm font-medium"><ReadableText>{title}</ReadableText></h4><p className="mt-2 whitespace-pre-wrap break-words text-sm leading-relaxed text-muted-foreground">{text}</p></section>
}

function RunOutput({run}: {run: CheckRun}) {
  return <section className="space-y-3" aria-label="Actual test output">{(run.state !== "finished" || run.outcome !== "pass") && <><h4 className="font-medium">Actual run result · {run.state}</h4><p className="text-sm">{run.outcome ?? 'No outcome yet'}{run.exitCode !== undefined ? ` · exit ${run.exitCode}` : ''}{run.durationMs !== undefined ? ` · ${run.durationMs} ms` : ''}</p></>}{run.reason && run.outcome !== "pass" && <p className="text-sm"><ReadableText>{run.reason}</ReadableText></p>}{(['stdout','stderr'] as const).map(key => <section key={key}><h5 className="text-sm font-medium">{key === 'stdout' ? 'Standard output' : 'Error output'}{run[key]?.truncated ? ' · truncated' : ''}</h5><pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-muted/30 p-3 text-xs">{run[key]?.status === 'intact' ? run[key]?.text || '(empty)' : active(run) ? 'Output will be available when this run finishes.' : 'Output not captured or its integrity could not be verified.'}</pre></section>)}<details><summary className="cursor-pointer text-xs">Run provenance</summary><p className="text-xs">{run.state} · {run.outcome ?? "No outcome yet"}{run.exitCode !== undefined ? ` · exit ${run.exitCode}` : ""}{run.durationMs !== undefined ? ` · ${run.durationMs} ms` : ""}</p><p className="text-xs text-muted-foreground">Started {run.startedAt}{run.finishedAt ? ` · Finished ${run.finishedAt}` : ''}</p><p className="mt-2 break-all text-xs">Source: {run.sourceDigest}<br/>Check: {run.checkDigest}<br/>Directory: {run.cwd}</p><pre className="whitespace-pre-wrap break-words text-xs">{run.command}</pre></details></section>
}
