...
export const OPERATION_SCHEMAS = {
  ...
  run_saved_check: z.object({ requestId, bindingId: id, expectedSourceDigest: digest, expectedCheckDigest: digest }).strict(),
  get_check_run: z.object({ runId: z.string().uuid() }).strict(),
  get_check_evidence: z.object({ bindingId: id }).strict(),
