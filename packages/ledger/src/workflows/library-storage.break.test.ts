...
import { mkdtempSync, readdirSync, mkdirSync, writeFileSync, readFileSync, symlinkSync, rmSync, existsSync } from 'node:fs'
...
const template=(root:string)=>{const {id,...input}=libraryTemplate(root);void id;return input}
...
test('library refuses symlinked profile storage without touching external files',()=>{
 ...
 try {
  ...
  assert.throws(()=>executeOperation(root,'save_workflow_profile',{...meta(),profile:template(root)}),/symlink/)
  assert.equal(readFileSync(join(outside,'sentinel'),'utf8'),'unchanged');assert.deepEqual(readdirSync(outside),['sentinel'])
...
test('a refused delete with an unsafe default pointer preserves the saved workflow',()=>{
 ...
 try {
  const saved=executeOperation(root,'save_workflow_profile',{...meta(),profile:template(root)}) as any,profile=saved.value
...
test('local library poison and cross-site requests cannot create profiles',async()=>{
 ...
 try{
  ...
  const body=JSON.stringify({action:'save',input:{...meta(),profile:template(root)}})
