...
function fixture(script="console.log('ok')") {const root=mkdtempSync(join(tmpdir(),'sl-run-break-'));initLedger(root,'breaker');writeFileSync(join(root,'check.cjs'),script);writeJson(join(root,'.spec-ledger/claims/e2aa3200-7e6e-5a69-9ce1-e93087fc2903.json'),{id:'e2aa3200-7e6e-5a69-9ce1-e93087fc2903',statement:'check',required:true});writeJson(join(root,'.spec-ledger/bindings/72ee57a1-e531-5e3d-9784-ceb630025dd7.json'),{id:'72ee57a1-e531-5e3d-9784-ceb630025dd7',claimId:'e2aa3200-7e6e-5a69-9ce1-e93087fc2903',kind:'test',locator:{type:'command',command:'node check.cjs'}});return root}
function input(root:string,id='breaker-request-1'){const e=getCheckEvidence(root,'72ee57a1-e531-5e3d-9784-ceb630025dd7');return{requestId:id,bindingId:'72ee57a1-e531-5e3d-9784-ceb630025dd7',expectedSourceDigest:e.sourceDigest!,expectedCheckDigest:e.checkDigest}}
...
test('source changes during a saved check cannot produce a passing outcome',async()=>{const root=fixture("require('fs').writeFileSync('changed.txt','changed');console.log('ok')");try{const r=executeOperation(root,'run_saved_check',input(root)) as CheckRun;assert.equal((await finish(root,r.runId)).outcome,'missing');assert.notEqual(getCheckEvidence(root,'72ee57a1-e531-5e3d-9784-ceb630025dd7').currentOutcome,'pass')}finally{rmSync(root,{recursive:true,force:true})}})
...
test('overlapping new requests are rejected while retry returns the original identity',async()=>{const root=fixture("setTimeout(()=>console.log('finished'),350)");try{const i=input(root),r=executeOperation(root,'run_saved_check',i) as CheckRun;assert.equal((executeOperation(root,'run_saved_check',i) as CheckRun).runId,r.runId);assert.throws(()=>executeOperation(root,'run_saved_check',{...i,requestId:'second-request-1'}),/owns this checkout|busy/);await finish(root,r.runId);assert.equal(getCheckEvidence(root,'72ee57a1-e531-5e3d-9784-ceb630025dd7').runs.length,1)}finally{rmSync(root,{recursive:true,force:true})}})
test('local malformed and cross-origin actions cause no execution',async()=>{const root=fixture();try{const bridge=createLocalCheckBridge(root),url='http://127.0.0.1:3737/api/checks',token=(await(await bridge(new Request(url))).json()).token;const headers={origin:'http://127.0.0.1:3737','content-type':'application/json','x-spec-ledger-token':token};for(const [h,body] of [[{...headers,origin:'http://evil.invalid'},JSON.stringify(input(root))],[{...headers,'x-spec-ledger-token':'wrong'},JSON.stringify(input(root))],[headers,JSON.stringify({...input(root),cwd:'/tmp'})],[headers,'x'.repeat(5000)]] as const){assert.ok((await bridge(new Request(url,{method:'POST',headers:h,body}))).status>=400)}assert.equal(getCheckEvidence(root,'72ee57a1-e531-5e3d-9784-ceb630025dd7').runs.length,0)}finally{rmSync(root,{recursive:true,force:true})}})
...
test('timeout terminates the owned command before releasing the guard and records signal',async()=>{
 ...
 try{
  ...
  writeJson(join(base,`${runId}.json`),{runId,requestId:i.requestId,bindingId:'72ee57a1-e531-5e3d-9784-ceb630025dd7',state:'queued',command:'node check.cjs',cwd:root,sourceDigest:i.expectedSourceDigest,checkDigest:i.expectedCheckDigest,startedAt:new Date().toISOString()})
