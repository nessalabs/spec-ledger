...
name: review-copy
description: >-
  Spec Ledger UI-facing review copy: one-sentence plainSummary / plainImpact.
  Use when writing any spec-break, code-break, align approve, or human review JSON.
---

# Review copy (Spec Ledger UI)

Every review JSON **must** satisfy [`schemas/review.json`](../../schemas/review.json):

| Field | Who sees it | Rule |
| --- | --- | --- |
| `plainSummary` | Spec Ledger UI Trail (default) | **Required.** One sentence, ≤280 chars. End behavior or takeaway. |
| `summary` | Technical notes (collapsed) | Required. Internals, test ids, paths OK. |
| `plainImpact` | Spec Ledger UI finding row | **Required on every finding.** One sentence of what would happen if this stands. |
...
node packages/ledger/dist/cli/main.js review add \
  ...
  --summary "technical…" \
  --plain-summary "one Spec Ledger UI sentence…"
...
Pre-commit validates staged review JSON for Spec Ledger UI copy + code-break evidence rules.
...
6. Spec Ledger UI Trail shows `plainSummary` / `plainImpact` first; technical notes stay collapsed.
7. Gates (history plane — not `verify.ok`):
   - `writeReview` / CLI refuse without Spec Ledger UI copy
