...
For a fixup that changes or clarifies the original spec, follow [repository correction links](../../docs/architecture/issue-intake.md#find-corrections-from-the-original-spec): add a dated correction document, link it near the top of the original spec, include back-links and the existing change record, preserve prior history, and verify the relative links. UI-only links are insufficient. Record the sealed-document amendment through the CLI.
