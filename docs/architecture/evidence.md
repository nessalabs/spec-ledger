...
Visual slices declare named screens, states and viewports in `trust.visualEvidence[sliceId]` and include `screenshot` in the slice evidence kinds. `record_screenshot` (CLI: `evidence screenshot`) records an open turn's local PNG/JPEG with current source/revision and computed file integrity; callers cannot supply a successful coverage verdict. `check_visual_evidence` (CLI: `evidence check`) evaluates each required surface without writing or running checks. A separate `visualEvidence` field accompanies `run_checks`; CLI `check` fails when that coverage is missing. Claims-only `verify.ok` semantics are unchanged.

Turn close requires coverage attached to that visual turn. Completion reevaluates coverage across the workstream, including closed turns, against the current source, plan and image bytes. A later authorized turn can explicitly refresh an earlier slice. Distinct surfaces require distinct image contents; changed, old unbound, remote, missing, invalid or duplicated screenshots cannot satisfy all coverage. Nonvisual and legacy plans keep their prior behavior; the planning and review skills require visual declarations when UI changes.

Capture beneath `.spec-ledger/evidence/screenshots/` to avoid changing source identity while recording images. The recorder uses the application permission/guard/retry path; the projection server remains read-only. The machine checks declared coverage and integrity; the reviewer judges whether screenshots actually demonstrate the UI. See [required screenshot workflow](../../skills/references/visual-evidence.md).
